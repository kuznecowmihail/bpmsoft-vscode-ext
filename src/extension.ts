...
/** Resolves which descriptor.json (if any) a just-*saved* package file should
 * bump `ModifiedOnUtc` for, and does it — resolves the save path via
 * `parsePkgPath`, then bumps `{itemDir}/descriptor.json` for
 * Schemas/SqlScripts/Data (including nested files such as
 * `Data/{Name}/Localization/…`, which still map to `Data/{Name}/descriptor.json`
 * via `itemDir`, and saving the item's own `descriptor.json` bumps that same
 * file's `ModifiedOnUtc`); for Resources bumps the owning schema's descriptor via
 * `findOwningSchemaDescriptor`; saving the package-level
 * `Pkg/{Package}/descriptor.json` bumps that same file; skips Files, Assemblies,
 * and anything else without a package `itemDir`. Only ever called from
...
function touchSchemaModifiedOnUtcForSavedFile(fsPath: string): void {
	const info = parsePkgPath(fsPath);
	if (!info) {
		return;
	}
	if (info.category === "Resources") {
		const ownerDescriptor = findOwningSchemaDescriptor(fsPath);
		if (ownerDescriptor) {
	...
	if (info.itemDir && (info.category === "Schemas" || info.category === "SqlScripts" || info.category === "Data")) {
		touchSchemaModifiedOnUtc(path.join(info.itemDir, "descriptor.json"));
		return;
	}
	const normalized = fsPath.replace(/\\/g, "/");
	if (/\/Pkg\/[^/]+\/descriptor\.json$/i.test(normalized)) {
		touchSchemaModifiedOnUtc(fsPath);
	}
}
...
async function rebuildWithProgress(forceFresh = false): Promise<void> {
	...
	await indexer.rebuild(forceFresh);
