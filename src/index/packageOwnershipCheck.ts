...
/**
 * "Boxed" package (naming-guidelines.md's "Коробочные пакеты" — stock/
 * vendor, or an installed package shipping only compiled output, with no
 * source to edit) — when it has no package-level `descriptor.json` of its
 * own. Confirmed against a full audit of both real installs (43 + 29
 * packages): every package that's genuinely unlocked for editing (real
 * `Schemas`/`Data`/`Resources`/`SqlScripts` content) has one; every
 * `Files`-only package (sometimes + its own `Autogenerated` — an installed
 * package's compiled schemas, e.g. `CentrexTelephony`/`OpenIdAuth`) doesn't.
 * The one shared source of truth for "is this package ours" — reused by
 * `PackagesTreeProvider` (Packages/Коробочные grouping) and `ModuleIndexer`
 * (which content is safe to cache wholesale between activations), so the
 * two can't disagree.
 */
export function isBoxedPackage(packageDir: string): boolean {
	return !fs.existsSync(path.join(packageDir, "descriptor.json"));
}
