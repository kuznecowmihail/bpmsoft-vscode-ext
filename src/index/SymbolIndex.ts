...
function schemaOriginKey(filePath: string): string {
...
export class SymbolIndex {
	...
	private modulesNamed(...names: string[]): IndexedModule[] {
		...
		for (const name of names) {
			out.push(...this.getAllByName(name));
		}
		return out;
	}
	...
	resolveThisMembers(filePath: string): IndexedMember[] {
		...
		this.appendModalBoxViewModelMembers(chainMods, result, seen);
	...
	/**
	 * Methods copied onto the schema VM by ModalBoxSchemaModule
	 * (`viewModel.foo = this.foo.bind(this)` in createViewModel).
	 */
	private appendModalBoxViewModelMembers(
		owners: IndexedModule[],
		result: IndexedMember[],
		seen: Set<string>
	): void {
		for (const member of this.modalBoxViewModelMembers(owners)) {
			this.pushUnseen(result, seen, member);
		}
	}
