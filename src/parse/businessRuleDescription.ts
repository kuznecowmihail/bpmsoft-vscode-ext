 ...
 * the editor hover, and the always-on inlay hint. Kept free of any
 * `SymbolIndex`/workspace dependency so it works the same from a pure-AST
 * context as from a context that does have one (the hover/inlay-hints
 * providers, which can pass `resolveComparisonType` for a fully symbolic
