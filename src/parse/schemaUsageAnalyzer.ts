...
/** `diff` item with `operation: "insert"` named like `GeneralInfo9db678f5-...`
 * — the Designer's autogenerated default, never renamed by the developer.
 * Only insert is checked: merge/move/remove inherit the name from a parent
 * schema insert. Flags it and, when a real prefix survives stripping the
 * GUID, offers to rename to just that (the field name is usually still a
 * perfectly good name on its own); a bare-GUID name (no prefix) is flagged
 * with no autofix since there's nothing sensible to default to. */
